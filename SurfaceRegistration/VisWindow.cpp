  ...
  public:
	...
	virtual void OnKeyPress() 
	{
		...
		if (key == "s")
		{
			...
			cout << "Please enter mesh (1 or 2): ";
			cin >> str;
			selectedMesh = atoi(str);
			cout << "Mesh selected is " << selectedMesh << "\n";

			cout << "Please enter point id: ";
			cin >> str;
			selectedPoint = atoi(str);
			cout << "Point selected is " << selectedPoint << "\n";
			...
			if (selectedMesh == 1)
			{
				...
				for (vtkIdType i = 0; i < this->meshreg->vtkMesh1->GetNumberOfPoints(); i++)
				{
					...
					float white[4] = {  255.0,255.0,255.0,255.0 };
					float red[4] = {  255.0,0.0,0.0,255.0 };

					if (i == selectedPoint)
						colors->SetTuple(i, red);
					else
						colors->SetTuple(i, white);
				}
				this->meshreg->vtkMesh1->GetPointData()->SetScalars(colors); 
				mesh1Mapper->SetScalarModeToDefault();
				mesh1Mapper->SetScalarVisibility(1);
			...
			else
			{
				vtkUnsignedCharArray* colors = vtkUnsignedCharArray::New();
				colors->SetNumberOfComponents(4);
				colors->SetNumberOfTuples(this->meshreg->vtkMesh2->GetNumberOfPoints());
				for (vtkIdType i = 0; i < this->meshreg->vtkMesh2->GetNumberOfPoints(); i++)
				{
					double point[3];
					this->meshreg->vtkMesh2->GetPoint(i, point);
				...
				this->meshreg->vtkMesh2->GetPointData()->SetScalars(colors); 
				mesh2Mapper->SetScalarModeToDefault();
				mesh2Mapper->SetScalarVisibility(1);
				mesh2Mapper->Update();
				mesh2Mapper->Modified();
		...
		if (key == "l")
		{
			...
			//if (this->meshreg->candidates[selectedPoint].size() != 0)
			{
				...
				while (node->level > 0)
				{
					if (this->meshreg->candidates[node->id].size() > 0)
						printf("%d -> %d\n", node->id, this->meshreg->candidates[node->id][0].id);
					else
						printf("%d -> -1\n", node->id);

					this->meshreg->PrintCandidates(node->id, 1);
		...
		if (key == "u")
		{
			...
			for (vtkIdType i = 0; i < this->meshreg->vtkMesh2->GetNumberOfPoints(); i++)
			{
				...
				// assign white to all points
				float blank[4] = {  255.0,255.0,255.0,255.0 };

				if ((this->meshreg->m2m1 != NULL) && (this->meshreg->m2m1[i] != -1))
				{
					int id1 = this->meshreg->m2m1[i];
					double* c1 = colors->GetTuple(id1);
					blank[0] = c1[0];
					blank[1] = c1[1];
					blank[2] = c1[2];
					blank[3] = c1[3];
...
void VisWindow::InitWindow()
{
	...
	transMesh2[0] = -0.0 * (bounds[1] - bounds[0]);
	transMesh2[1] = 0.0 * (bounds[3] - bounds[2]);
	transMesh2[2] = 1.5 * (bounds[5] - bounds[4]);
