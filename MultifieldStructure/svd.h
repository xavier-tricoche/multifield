...
inline double pythag(double a, double b)
{
    ...
    fabsa=fabs(a);
    fabsb=fabs(b);
    if (fabsa > fabsb) return fabsa*sqrt(1.0+SQR(fabsb/fabsa));
    else return (fabsb == 0.0 ? 0.0 : fabsb*sqrt(1.0+SQR(fabsa/fabsb)));
}
...
inline nvis::fixed_matrix<T, N> to_mat(const nvis::fixed_vector<T, N>& w) {
	...
	for (int i=0 ; i<N ; ++i) r(i,i) = w[i];
	return r;
}

template<typename T, int N>
inline nvis::fixed_matrix<T, N> to_inv_mat(const nvis::fixed_vector<T, N>& w) {
	nvis::fixed_matrix<T, N> r = nvis::fixed_matrix<T, N>::identity();
	for (int i=0 ; i<N ; ++i) r(i,i) = (w[i] > 0 ? 1./w[i] : 0);
	return r;
}

template<typename T, int M, int N>
inline nvis::fixed_matrix<T, M, N> pseudoinv(const nvis::fixed_matrix<T, M, N>& A) {
	nvis::fixed_matrix<T, M, N> U;
	nvis::fixed_matrix<T, N> V;
	nvis::fixed_vector<T, N> w;
	svdcmp<T, M, N>(A, w, U, V); 
    return V*to_inv_mat<double, 3>(w)*nvis::transpose(U);
}
