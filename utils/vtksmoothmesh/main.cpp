...
int main( int argc, char** argv ) 
{
	if (argc != 6)
	{
		printf("Example: vtksmoothmesh int.vtk out.vtk 20 120.0 0.1\n");
		return 0;
	}
	...
	// decimation
	vtkSmartPointer<vtkWindowedSincPolyDataFilter> deci;
	deci = vtkSmartPointer<vtkWindowedSincPolyDataFilter>::New();
	deci->SetInput(m);
	deci->SetNumberOfIterations(atoi(argv[3]));
	deci->BoundarySmoothingOn();
	deci->FeatureEdgeSmoothingOn();
	deci->SetFeatureAngle(atof(argv[4]));
	deci->SetPassBand(atof(argv[5]));
	deci->NonManifoldSmoothingOn();
	deci->NormalizeCoordinatesOn();
	deci->Update();
