...
int main( int argc, char** argv ) 
{
	if (argc != 3)
	{
		printf("Example: vtkcomputenormals int.vtk out.vtk\n");
		return 0;
	}

	cout << "=================\n";
	cout << "vtkcomputenormals\n";
	cout << "=================\n";
	...
	// decimation
	vtkSmartPointer<vtkPolyDataNormals> deci;
	deci = vtkSmartPointer<vtkPolyDataNormals>::New();
	deci->SetInput(m);
	deci->NonManifoldTraversalOn();
	deci->ComputePointNormalsOn();
	deci->ConsistencyOn();
	deci->SplittingOff();
	deci->Update();
