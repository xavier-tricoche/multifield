...
int main (int argc, char *argv[])
{
	if (argc < 10)
	{
		printf("Example: vtkfindbbox from.vtk to.vtk minfx minfy minfz maxfx maxfy maxfz file.nrrd\n");
		return 0;
	}
	
	Nrrd* nrrdfile = readNrrd(argv[9]);

	float3 minf = make_float3((float) atof(argv[3]), (float) atof(argv[4]), (float) atof(argv[5]));
	float3 maxf = make_float3((float) atof(argv[6]), (float) atof(argv[7]), (float) atof(argv[8]));
