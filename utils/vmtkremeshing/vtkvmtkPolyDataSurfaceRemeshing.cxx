  ...
  vtkInformationVector *outputVector)
{
  ...
  int i = 0; 
  vtkCellArray* acells = input->GetPolys();
  acells->InitTraversal();
  vtkIdType npts;
  vtkIdType* pts;
  while (acells->GetNextCell(npts, pts))
    {

	input->GetPoint(pts[0], point1);
	input->GetPoint(pts[1], point2);
	input->GetPoint(pts[2], point3);

	//cellNormals->GetTuple(i,orientedCellNormal);
	//printf("%lf %lf %lf, ", orientedCellNormal[0], orientedCellNormal[1], orientedCellNormal[2]);

    //vtkTriangle::ComputeNormal(point1,point2,point3,cellNormal); 
	//printf("%lf %lf %lf, ", cellNormal[0], cellNormal[1], cellNormal[2]);

	//printf("%lf\n", vtkMath::Dot(cellNormal,orientedCellNormal));
    //fflush(stdout);
    ...
    //if (vtkMath::Dot(cellNormal,orientedCellNormal) > 0.0)
     // {
      workingCells->InsertCellPoint(pts[0]);
      workingCells->InsertCellPoint(pts[1]);
      workingCells->InsertCellPoint(pts[2]);
    //  }
   // else
     // {
