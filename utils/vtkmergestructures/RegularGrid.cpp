...
RegularGrid::RegularGrid(Nrrd* _ni, int _width, int _height, int _depth, double spcx, double spcy, double spcz)
{
	...
	data = (float*) malloc(width * height* depth * sizeof(float));
}
...
bool RegularGrid::InDomain(const float3& spt)
{
...
float3 RegularGrid::Space2Grid(const float3& spt)
{
	float3 gpt;
	
	gpt.x = (width - 1) * (spt.x - xs[0]) / (xs[width - 1] - xs[0]);
	gpt.y = (height - 1) * (spt.y - ys[0]) / (ys[height - 1] - ys[0]);
	gpt.z = (depth - 1)  * (spt.z - zs[0]) / (zs[depth - 1] - zs[0]);

	return gpt;
}

float3 RegularGrid::Grid2Space(const float3& gpt)
{
	float3 spt;

	spt.x = xs[0] + (xs[width - 1] - xs[0]) * gpt.x / (width - 1);
	spt.y = ys[0] + (ys[height - 1] - ys[0]) * gpt.y / (height - 1);
	spt.z = zs[0] + (zs[depth - 1] - zs[0])  * gpt.z / (depth - 1);
...
{	
	...
	*scalar = GetValueAt((double) pt.x, (double) pt.y, (double) pt.z);
